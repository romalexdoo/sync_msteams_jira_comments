...
pub async fn handler(
    ...
    let signature = get_signature_from_headers(headers).map_err(ApiError::c500).context("Failed to get signature")?;

    validate_signature(
            &payload, 
            &jira_api.config.secret, 
            &signature
        )
        .map_err(ApiError::c500)
        .context("Failed to validate signature")?;
    
    let json_payload = serde_json::from_slice::<Value>(&payload).map_err(ApiError::c500).context("Failed to deserialize payload")?;
    let request = serde_json::from_slice::<Request>(&payload).map_err(ApiError::c500).context("Failed to deserialize payload")?;
    ...
    if let Some(link) = teams_link {
        if request.changelog.items.iter().any(|i| i.field.to_lowercase() == String::from("status")) {
            if let Some(message_id) = extract_message_id_from_url(link.to_string()) {
                ...
                graph_api.reply_to_issue(&message_id, &reply_body).await.map_err(ApiError::c500).context("Failed to send notification to the channel")?;
            }
        }
    }
...
fn validate_signature(payload: &Bytes, secret: &String, signature: &Signature) -> Result<()> {
    ensure!(signature.method == "sha256".to_string(), "Wrong method, expected: sha256, got: {}", signature.method);
